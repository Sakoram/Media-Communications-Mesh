...
#define DEFAULT_RECV_IP "192.168.1.85"
#define DEFAULT_RECV_PORT "9002"
#define DEFAULT_SEND_IP "192.168.1.85"
...
static int send_frame(mcm_conn_context* dp_ctx_tx, char *data, size_t data_size, size_t buf_size) {
    mcm_buffer* buf = NULL;

    buf = mcm_dequeue_buffer(dp_ctx_tx, -1, NULL);
    if (buf == NULL) {
        return 1;
    }

    printf("#@#@#@ resending the buf\n");

    memcpy(buf->data, data, data_size);
    buf->len = buf_size;


    if (mcm_enqueue_buffer(dp_ctx_tx, buf) != 0) {
        return 1;
    }
    return 0;
}
...
int main(int argc, char** argv)
{
    ...
    mcm_conn_context* dp_ctx_rx = NULL, *dp_ctx_tx = NULL;
    ...
    /* is receiver */
    param.type = is_rx;
    dp_ctx_rx = mcm_create_connection(&param);
    if (dp_ctx_rx == NULL) {
    ...
    /* is sender */
    param.type = is_tx;
    dp_ctx_tx = mcm_create_connection(&param);
    if (dp_ctx_tx == NULL) {
        printf("Fail to connect to MCM data plane\n");
        exit(-1);
    }
    ...
    while (keepRunning) {
        ...
        buf = mcm_dequeue_buffer(dp_ctx_rx, timeout, &err);
        if (buf == NULL) {
            if (err == 0) {
        ...
        ts_send = *(struct timespec *)buf->data;
        latency_us = 1000000.0 * (ts_recv.tv_sec - ts_send.tv_sec);
        latency_us += (ts_recv.tv_nsec - ts_send.tv_nsec) / 1000.0;
        printf("before respond | frames: [%u], latency: %0.1lf us\n", frame_count, latency_us);

        send_frame(dp_ctx_tx, buf->data, sizeof(struct timespec), frm_size);

        clock_gettime(CLOCK_REALTIME, &ts_recv);
        latency_us = 1000000.0 * (ts_recv.tv_sec - ts_send.tv_sec);
        latency_us += (ts_recv.tv_nsec - ts_send.tv_nsec) / 1000.0;
        printf("after respond | frames: [%u], latency: %0.1lf us\n", frame_count, latency_us);


        frame_count++;

        /* enqueue buffer */
        if (mcm_enqueue_buffer(dp_ctx_rx, buf) != 0) {
            break;
        }
        printf("\n");
    }
    ...
    printf("Destroy MCM connections\n");
    mcm_destroy_connection(dp_ctx_rx);
    mcm_destroy_connection(dp_ctx_tx);
